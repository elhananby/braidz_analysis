...
from braidz_writer import dir_to_braidz, BraidzError
...
def process_braid_folder(braid_path: Path) -> Tuple[bool, str]:
    ...
    try:
        ...
        if not is_valid_braidz(braidz_path):
            raise CleanupError(f"Created .braidz file is invalid")
...
def main(base_dir: Union[str, Path]) -> None:
    ...
    logging.basicConfig(
        ...
        handlers=[
            ...
            logging.FileHandler('braid_cleanup.log')
        ]
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "base_dir",
        type=str,
        help="Base directory containing .braid folders"
    )
