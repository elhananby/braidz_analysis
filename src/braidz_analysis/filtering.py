...
def filter_by_time(
    ...
    logger.info(f"Time filter: {len(valid_groups)} trajectories passed "
...
def filter_by_distance(
    ...
    logger.info(f"Distance filter: {len(valid_groups)} trajectories passed "
...
def filter_by_median(
    ...
    logger.info(f"Median position filter: {len(valid_groups)} trajectories passed "
...
def find_common_trajectories(*args) -> List[Tuple[int, int]]:
    ...
    logger.info(f"Found {len(common_trajectories)} trajectories common to all {len(args)} filters")
